<?php
	 ...
	 * listing a collection of Team objects.  It includes
	 ...
	 * 
	 */
	class TeamDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Team datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Team object, or it can be any QQNode extending from QQN::Team().
		 * 
		 ...
		 * sorting using two fields from the Team object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Team
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Team::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Team, given the clauses above
			$this->DataSource = Team::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Team-based QQNode.
		 * It will also verify that it is a proper Team-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'team') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "team".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Team()->Id;
				case 'Name': return QQN::Team()->Name;
				case 'Members': return QQN::Team()->Members;
				case 'CreatedAt': return QQN::Team()->CreatedAt;
				default: throw new QCallerException('Simple Property not found in TeamDataGrid content: ' . $mixContent);
