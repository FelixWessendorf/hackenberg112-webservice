<?php
	...
	class PersonDataGridGen extends QDataGrid {
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intPersonId=<?=urlencode($_ITEM->PersonId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->PersonId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->PersonId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'PersonId': return QQN::Person()->PersonId;
				case 'Sex': return QQN::Person()->Sex;
				case 'FirstName': return QQN::Person()->FirstName;
				case 'LastName': return QQN::Person()->LastName;
				case 'EMailAddress': return QQN::Person()->EMailAddress;
				case 'DateOfBirth': return QQN::Person()->DateOfBirth;
				case 'DateOfEntry': return QQN::Person()->DateOfEntry;
				case 'RankId': return QQN::Person()->RankId;
				case 'Rank': return QQN::Person()->Rank;
				case 'Active': return QQN::Person()->Active;
