<?php
	 ...
	 * listing a collection of Booking objects.  It includes
	 ...
	 * 
	 */
	class BookingDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Booking datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Booking object, or it can be any QQNode extending from QQN::Booking().
		 * 
		 ...
		 * sorting using two fields from the Booking object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Booking
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Booking::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Booking, given the clauses above
			$this->DataSource = Booking::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Booking-based QQNode.
		 * It will also verify that it is a proper Booking-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'booking') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "booking".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Booking()->Id;
				case 'TeamId': return QQN::Booking()->TeamId;
				case 'Team': return QQN::Booking()->Team;
				case 'Amount': return QQN::Booking()->Amount;
				case 'CreatedAt': return QQN::Booking()->CreatedAt;
				default: throw new QCallerException('Simple Property not found in BookingDataGrid content: ' . $mixContent);
