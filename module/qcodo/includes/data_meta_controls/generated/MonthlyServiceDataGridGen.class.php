<?php
	 ...
	 * listing a collection of MonthlyService objects.  It includes
	 ...
	 * 
	 */
	class MonthlyServiceDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this MonthlyService datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the MonthlyService object, or it can be any QQNode extending from QQN::MonthlyService().
		 * 
		 ...
		 * sorting using two fields from the MonthlyService object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from MonthlyService
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intMonth=<?=urlencode($_ITEM->Month)?>&intPersonId=<?=urlencode($_ITEM->PersonId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->Month))?>/<?=str_replace("+","%20",urlencode($_ITEM->PersonId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Month . "," . $_ITEM->PersonId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = MonthlyService::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from MonthlyService, given the clauses above
			$this->DataSource = MonthlyService::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a MonthlyService-based QQNode.
		 * It will also verify that it is a proper MonthlyService-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'monthly_service') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "monthly_service".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Month': return QQN::MonthlyService()->Month;
				case 'PersonId': return QQN::MonthlyService()->PersonId;
				case 'Person': return QQN::MonthlyService()->Person;
				default: throw new QCallerException('Simple Property not found in MonthlyServiceDataGrid content: ' . $mixContent);
