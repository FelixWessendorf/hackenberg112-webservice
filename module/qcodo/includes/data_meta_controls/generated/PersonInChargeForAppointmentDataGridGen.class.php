<?php
	 ...
	 * listing a collection of PersonInChargeForAppointment objects.  It includes
	 ...
	 * 
	 */
	class PersonInChargeForAppointmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PersonInChargeForAppointment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PersonInChargeForAppointment object, or it can be any QQNode extending from QQN::PersonInChargeForAppointment().
		 * 
		 ...
		 * sorting using two fields from the PersonInChargeForAppointment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PersonInChargeForAppointment
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAppointmentId=<?=urlencode($_ITEM->AppointmentId)?>&intPersonId=<?=urlencode($_ITEM->PersonId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AppointmentId))?>/<?=str_replace("+","%20",urlencode($_ITEM->PersonId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AppointmentId . "," . $_ITEM->PersonId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PersonInChargeForAppointment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PersonInChargeForAppointment, given the clauses above
			$this->DataSource = PersonInChargeForAppointment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PersonInChargeForAppointment-based QQNode.
		 * It will also verify that it is a proper PersonInChargeForAppointment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'person_in_charge_for_appointment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "person_in_charge_for_appointment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AppointmentId': return QQN::PersonInChargeForAppointment()->AppointmentId;
				case 'Appointment': return QQN::PersonInChargeForAppointment()->Appointment;
				case 'PersonId': return QQN::PersonInChargeForAppointment()->PersonId;
				case 'Person': return QQN::PersonInChargeForAppointment()->Person;
				default: throw new QCallerException('Simple Property not found in PersonInChargeForAppointmentDataGrid content: ' . $mixContent);
