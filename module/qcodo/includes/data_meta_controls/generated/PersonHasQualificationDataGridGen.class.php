<?php
	 ...
	 * listing a collection of PersonHasQualification objects.  It includes
	 ...
	 * 
	 */
	class PersonHasQualificationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PersonHasQualification datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PersonHasQualification object, or it can be any QQNode extending from QQN::PersonHasQualification().
		 * 
		 ...
		 * sorting using two fields from the PersonHasQualification object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PersonHasQualification
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intPersonId=<?=urlencode($_ITEM->PersonId)?>&intQualificationId=<?=urlencode($_ITEM->QualificationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->PersonId))?>/<?=str_replace("+","%20",urlencode($_ITEM->QualificationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->PersonId . "," . $_ITEM->QualificationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PersonHasQualification::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PersonHasQualification, given the clauses above
			$this->DataSource = PersonHasQualification::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PersonHasQualification-based QQNode.
		 * It will also verify that it is a proper PersonHasQualification-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'person_has_qualification') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "person_has_qualification".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'PersonId': return QQN::PersonHasQualification()->PersonId;
				case 'Person': return QQN::PersonHasQualification()->Person;
				case 'QualificationId': return QQN::PersonHasQualification()->QualificationId;
				case 'Qualification': return QQN::PersonHasQualification()->Qualification;
				default: throw new QCallerException('Simple Property not found in PersonHasQualificationDataGrid content: ' . $mixContent);
