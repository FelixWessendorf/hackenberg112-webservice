<?php
	 ...
	 * listing a collection of Rank objects.  It includes
	 ...
	 * 
	 */
	class RankDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Rank datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Rank object, or it can be any QQNode extending from QQN::Rank().
		 * 
		 ...
		 * sorting using two fields from the Rank object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Rank
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intRankId=<?=urlencode($_ITEM->RankId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->RankId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->RankId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Rank::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Rank, given the clauses above
			$this->DataSource = Rank::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Rank-based QQNode.
		 * It will also verify that it is a proper Rank-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'rank') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "rank".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'RankId': return QQN::Rank()->RankId;
				case 'Name': return QQN::Rank()->Name;
				case 'Sort': return QQN::Rank()->Sort;
				case 'Sex': return QQN::Rank()->Sex;
				default: throw new QCallerException('Simple Property not found in RankDataGrid content: ' . $mixContent);
