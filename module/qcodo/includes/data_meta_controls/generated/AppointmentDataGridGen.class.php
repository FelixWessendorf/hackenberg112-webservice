<?php
	 ...
	 * listing a collection of Appointment objects.  It includes
	 ...
	 * 
	 */
	class AppointmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Appointment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Appointment object, or it can be any QQNode extending from QQN::Appointment().
		 * 
		 ...
		 * sorting using two fields from the Appointment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Appointment
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAppointmentId=<?=urlencode($_ITEM->AppointmentId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AppointmentId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AppointmentId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Appointment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Appointment, given the clauses above
			$this->DataSource = Appointment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Appointment-based QQNode.
		 * It will also verify that it is a proper Appointment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'appointment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "appointment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AppointmentId': return QQN::Appointment()->AppointmentId;
				case 'Start': return QQN::Appointment()->Start;
				case 'End': return QQN::Appointment()->End;
				case 'Description': return QQN::Appointment()->Description;
				case 'Location': return QQN::Appointment()->Location;
				case 'AdditionalInformation': return QQN::Appointment()->AdditionalInformation;
				default: throw new QCallerException('Simple Property not found in AppointmentDataGrid content: ' . $mixContent);
