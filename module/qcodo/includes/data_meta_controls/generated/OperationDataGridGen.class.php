<?php
	 ...
	 * listing a collection of Operation objects.  It includes
	 ...
	 * 
	 */
	class OperationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Operation datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Operation object, or it can be any QQNode extending from QQN::Operation().
		 * 
		 ...
		 * sorting using two fields from the Operation object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Operation
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intOperationId=<?=urlencode($_ITEM->OperationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->OperationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OperationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Operation::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Operation, given the clauses above
			$this->DataSource = Operation::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Operation-based QQNode.
		 * It will also verify that it is a proper Operation-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'operation') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "operation".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'OperationId': return QQN::Operation()->OperationId;
				case 'Date': return QQN::Operation()->Date;
				case 'Description': return QQN::Operation()->Description;
				default: throw new QCallerException('Simple Property not found in OperationDataGrid content: ' . $mixContent);
