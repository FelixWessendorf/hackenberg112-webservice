<?php
	 ...
	 * listing a collection of Qualification objects.  It includes
	 ...
	 * 
	 */
	class QualificationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Qualification datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Qualification object, or it can be any QQNode extending from QQN::Qualification().
		 * 
		 ...
		 * sorting using two fields from the Qualification object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Qualification
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intQualificationId=<?=urlencode($_ITEM->QualificationId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->QualificationId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->QualificationId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Qualification::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Qualification, given the clauses above
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Qualification-based QQNode.
		 * It will also verify that it is a proper Qualification-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'qualification') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "qualification".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'QualificationId': return QQN::Qualification()->QualificationId;
				case 'Name': return QQN::Qualification()->Name;
				default: throw new QCallerException('Simple Property not found in QualificationDataGrid content: ' . $mixContent);
